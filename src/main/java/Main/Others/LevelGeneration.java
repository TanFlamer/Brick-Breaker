...
public class LevelGeneration {
    ...
    private Brick[][] makeAllLevel(Rectangle drawArea, int brickRow, int level){
        ...
        for(int i = 0; i < brickTotal; i++){
            ...
            int rowNum = (i / twoRows) * randBrickRow;
            ...
            if((choice[level][0]-1)/4==0){
                ...
                if(i % twoRows < randBrickRow){ //even row
                    ...
                    if(choice[level][0]%4==0||choice[level][0]%4==3)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,brickRand[i%4]);
                    else if(choice[level][0]%4==1)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,brickChoice[i%(choice[level][3]+1)]+1);
                    else if(choice[level][0]%4==2)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,brickRand[i%(choice[level][3]+1)]);
                }
                else{ //odd row

                    int posX = i % twoRows - randBrickRow; //get position of brick on odd row
                    if(choice[level][0]%4==0||choice[level][0]%4==3)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,brickRand[i%4]);
                    else if(choice[level][0]%4==1)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,brickChoice[i%(choice[level][3]+1)]+1);
                    else if(choice[level][0]%4==2)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,brickRand[i%(choice[level][3]+1)]);
                }
            }
                ...
                if(i % twoRows < randBrickRow){ //even row
                    ...
                    if(choice[level][0]%4==0||choice[level][0]%4==3)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,random.nextInt(4)+1);
                    else if(choice[level][0]%4==1)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,brickChoice[random.nextInt(choice[level][3]+1)]+1);
                    else if(choice[level][0]%4==2)
                        tmp[0][rowNum + i % twoRows] = makeBrick(p,brickSize,brickRand[random.nextInt(choice[level][3]+1)]);
                }
                else{ //odd row

                    int posX = i % twoRows - randBrickRow; //get position of brick on odd row
                    if(choice[level][0]%4==0||choice[level][0]%4==3)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,random.nextInt(4)+1);
                    else if(choice[level][0]%4==1)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,brickChoice[random.nextInt(choice[level][3]+1)]+1);
                    else if(choice[level][0]%4==2)
                        tmp[1][rowNum + posX] = makeBrick(p,brickSize,brickRand[random.nextInt(choice[level][3]+1)]);
                }
            }
        }
        return tmp;
    ...
    private Brick[][] makeChessboardLevel(Rectangle drawArea, int typeA, int typeB, int level){
        ...
        for(int i = 0; i < 31; i++){
            ...
            if(i % twoRows < brickOnLine){ //even row
                tmp[0][rowNum + i % twoRows] = ((i % twoRows) % 2 == 0) ? makeBrick(p,brickSize,typeA) : makeBrick(p,brickSize,typeB);
            }
            else{ //odd row
                int posX = i % twoRows - brickOnLine; //get position of brick on odd row
                tmp[1][rowNum + posX] = ((posX > centerLeft && posX <= centerRight) || posX == 10) ? makeBrick(p,brickSize,typeA) : makeBrick(p,brickSize,typeB);
            }
        }
        return tmp;
    }
